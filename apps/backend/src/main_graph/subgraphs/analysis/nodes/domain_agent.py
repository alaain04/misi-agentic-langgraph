...
from src.main_graph.subgraphs.analysis.agents.registry import get_agent_prompt, get_agent_tools
...
async def domain_agent(state: AnalysisState, config: RunnableConfig) -> dict:
    ...
    prompt = get_agent_prompt(dispatch.agent_type)

    logger.info("domain_agent: domain=%s hypothesis=%s", dispatch.domain, dispatch.hypothesis[:60])
    bundle = await run_react_loop(dispatch, prep, tools, system_prompt=prompt)
