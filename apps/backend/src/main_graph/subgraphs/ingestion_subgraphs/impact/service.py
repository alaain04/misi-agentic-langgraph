"""Impact analysis service — pure business logic."""
...
async def analyze_service(state: ImpactState, dao: IngestionResultPort) -> dict:
    """Analyze impact of a dependency using agentic approach.

    Args:
        state: ImpactState containing dependency_name, repo_path, sbom_cyclonedx
        dao: IngestionResultPort for persistence

    Returns:
        dict with result_id key
    """
    ...
    try:
        ...
        _log.exception("impact.analyze_service: agent failed for dep=%s", dep_name)
        entry = ImpactEntry(dep_name=dep_name)

    result_id = await dao.save(entry)
    _log.info("impact.analyze_service: saved result_id=%s dep=%s", result_id, dep_name)
    return {"result_id": result_id}
