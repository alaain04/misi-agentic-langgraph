...
| 3.4 | Maintenance findings are absent for every transitive package with real evidence of staleness available (i.e. not just "none happened to be flagged") — cross-check by manually running `package_health_data` against a known-stale transitive in the target repo and confirming the agent still doesn't surface it as a finding | Q5, strongest form | not yet run — needs a repo with a deliberately old/abandoned transitive dependency to be a meaningful test, not just an absence-of-evidence result |
