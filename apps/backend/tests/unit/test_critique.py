...
def _dispatch() -> AgentDispatch:
    return AgentDispatch(
        ...
        hypothesis="express has a known CVE",
...
def test_format_findings_marks_missing_evidence():
    ...
    f = FindingNote(
        dep_name="express", severity="high", description="CVE-123", evidence=[]
    )
...
def test_format_findings_includes_snippets():
    ...
    f = FindingNote(
        ...
        description="CVE-123",
        evidence=[
            EvidenceRef(tool="npm_audit", url=None, log_snippet="advisory 1234 high")
        ],
    )
    rendered = _format_findings([f], {})
...
async def test_critique_findings_returns_verdict():
    from src.main_graph.subgraphs.analysis.agents.critique import (
        FindingsVerdict,
        critique_findings,
    )

    verdict = FindingsVerdict(
        ok=False, feedback="finding 1 lacks evidence", calibrated_confidence=0.2
    )
    mock_llm = MagicMock()
