...
from src.main_graph.constants import EXECUTION_PLANNER, SUMMARIZER
...
def stage_router(state: MainState) -> str:
    """Route to the next stage dispatch loop or to the summarizer when done."""
    idx = state.get("current_stage_index", 0)
    stages = state.get("execution_stages", [])
    return EXECUTION_PLANNER if idx < len(stages) else SUMMARIZER
